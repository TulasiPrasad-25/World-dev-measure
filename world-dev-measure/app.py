...
# Base directory — same folder as app.py (works locally & on Streamlit Cloud)
BASE = Path(__file__).parent

def load(filename):
    return pickle.load(open(BASE / filename, 'rb'))

# Load pickles
df         = load('df_display.pkl')
X_pca      = load('X_pca.pkl')
kmeans_lbl = load('kmeans_labels.pkl')
dbscan_lbl = load('dbscan_labels.pkl')
hc_lbl     = load('hc_labels.pkl')
df_scaled  = load('df_scaled.pkl')
wcss       = load('wcss.pkl')
